...
export default function Layout() {
  return (
    <div className="flex min-h-screen">
      <aside className="fixed inset-y-0 left-0 z-40 flex w-16 flex-col items-center gap-2 border-r border-white/10 bg-black/40 py-4 md:w-56 md:items-stretch md:px-4">
        <div className="mb-4 flex items-center gap-2 px-1">
          <img
            ...
            alt="KPC"
            className="hidden h-8 w-8 rounded md:block"
            onError={(e) => (e.currentTarget.style.display = "none")}
          />
          <div className="hidden md:block">
            <p className="text-sm font-bold text-white">KPC Yard CP</p>
            <p className="text-[10px] text-slate-500">MBA Depot</p>
          </div>
        </div>
      ...
      <main className="ml-16 flex-1 p-4 md:ml-56 md:p-6">
        <Outlet />
      </main>
    </div>
  );
